...
port="$(python3 - <<'PY'
import json
try: print(int(json.load(open('/etc/vps-control/tuic/settings.json')).get('port',8444)))
except Exception: print(8444)
PY
)"
rule=(-p udp --dport "${port}" -m comment --comment vps-control-tuic -j ACCEPT)
if [[ "${action}" == add ]]; then
