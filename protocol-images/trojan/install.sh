...
if [[ ! -s "$ROOT/server.crt" ]]; then openssl req -x509 -newkey rsa:3072 -sha256 -nodes -days 3650 -subj '/CN=endpoint.internal' -addext 'subjectAltName=DNS:endpoint.internal' -keyout "$ROOT/server.key" -out "$ROOT/server.crt"; fi; chmod 0600 "$ROOT"/server.*
[[ -s "$ROOT/settings.json" ]] || printf '{"port":%s}\n' "$PORT" >"$ROOT/settings.json"
[[ -s "$ROOT/config.json" ]] || python3 - "$ROOT/config.json" "$ROOT" "$PORT" <<'PY'
import json,os,sys
p,r,port=sys.argv[1],sys.argv[2],int(sys.argv[3]); c={'log':{'level':'warn'},'inbounds':[{'type':'trojan','tag':'trojan-in','listen':'::','listen_port':port,'users':[],'tls':{'enabled':True,'server_name':'endpoint.internal','certificate_path':f'{r}/server.crt','key_path':f'{r}/server.key'}}],'outbounds':[{'type':'direct','tag':'direct'}]}; open(p,'w').write(json.dumps(c,indent=2)); os.chmod(p,0o600)
PY
