        ...
        <section className="confirmDialog sshAdminDialog" role="dialog" aria-modal="true" aria-labelledby="ssh-admin-dialog-title" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div className="sshAdminBody">
            <section className="sshAdminStatus">
              ...
              <em>{sshAccessState?.phase === "hardened" ? "HARDENED" : sshAccessState?.phase === "awaiting-confirmation" ? "ROLLBACK ON" : sshPublicAccessOpen || sshAccessState?.phase === "open" ? "SSH OPEN" : "SAFE MODE"}</em>
            </section>
            <section className="sshAdminDirectAccess">
              <div><div className="sshAdminSectionTitle"><span>Публичный SSH-доступ</span><small>НЕЗАВИСИМО</small></div><p>{sshPublicAccessOpen || sshAccessState?.phase === "open" ? "SSH/22 разрешён извне, root доступен по паролю и ключу." : "Открывает SSH/22 извне и разрешает вход root по паролю и ключу."}</p></div>
              <button className="sshStepButton sshOpenButton" type="button" disabled={busy || sshAccessState?.phase === "awaiting-confirmation" || sshPublicAccessOpen || sshAccessState?.phase === "open"} onClick={() => void changeSshAccess("disable")}>{sshPublicAccessOpen || sshAccessState?.phase === "open" ? "Доступ уже открыт" : "Открыть SSH/22 и root"}</button>
            </section>
