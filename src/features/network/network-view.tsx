...
function routeTags(domain: NetworkStatus["domains"][number]) {
  ...
  if (!role.includes("udp") && (role.includes("vless") || role.includes("cdn") || role.includes("tls") || role.includes("panel") || role.includes("relay"))) add("TCP", "tcp");
...
function NetworkRouteTags({ domain }: { domain: NetworkStatus["domains"][number] }) {
  return <div className="networkRouteTags" aria-label="Тип маршрута">{routeTags(domain).map((tag) => <span className={`networkRouteTag ${tag.kind}`} key={tag.label}>{tag.label}</span>)}</div>;
}
...
function NetworkIdentityDetails({ domain }: { domain: NetworkStatus["domains"][number] }) {
  ...
  return <div className="networkIdentityBody">
    ...
    {domain.edge && (domain.edge.provider !== "Unknown" || domain.edge.cnames.length > 0) && <div className="networkIdentityDns"><span>CDN / edge</span><strong>{domain.edge.provider}</strong><small>{domain.edge.source}{domain.edge.cnames.length ? ` · ${domain.edge.cnames.join(", ")}` : ""}</small></div>}
    {addresses.length > 0 && <div className="networkIdentityIps">{addresses.map((address) => { const item = ipInfo.find((candidate) => candidate.address === address); return <div className="networkIdentityIp" key={address}><code>{address}</code><span>{item ? [item.hoster, item.asn].filter(Boolean).join(" · ") || "Unknown" : "Сведения об IP не получены"}</span>{item?.provider && item.provider !== item.hoster && <small>RDAP: {item.provider}</small>}{item?.ptr && <small>PTR: {item.ptr}</small>}</div>; })}</div>}
  </div>;
}
...
function DiagnosticsV2({ status, request, endpointDraft, endpointDirty, busy, setEndpointDraft, saveEndpoints }: { status: NetworkStatus; request: NetworkRequest; endpointDraft: NetworkEndpointSettings | null; endpointDirty: boolean; busy: boolean; setEndpointDraft: (value: NetworkEndpointSettings) => void; saveEndpoints: () => void }) {
  ...
  return <div className="networkV2">
    ...
    <section className="networkV2RouteBlock networkPanel" aria-label="Домены и IP серверов"><header className="networkV2BlockHead"><div><span className="networkKicker">МАРШРУТЫ</span><h2>Домены и IP серверов</h2><p>Нажмите на домен, чтобы раскрыть его DNS-записи, CDN-сигналы и IP.</p></div><div className="networkV2Actions"><label className="networkSearch"><span className="networkSrOnly">Найти адрес</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Найти адрес" /></label>{endpointDraft && <NetworkEndpoints request={request} draft={endpointDraft} busy={busy} dirty={endpointDirty} onChange={(key, value) => setEndpointDraft({ ...endpointDraft, [key]: value })} onSave={saveEndpoints} />}</div></header><div className="networkTableWrap"><table><thead><tr><th>Домен / IP</th><th>Роль</th><th>DNS / IP-сведения</th><th>Маршрут</th></tr></thead><tbody>{domains.map((domain) => { const rowKey = `${domain.role}-${domain.value}`; const expanded = expandedDomains.has(rowKey); return <Fragment key={rowKey}><tr className={expanded ? "networkRouteRow is-expanded" : "networkRouteRow"}><td><button type="button" className="networkRouteExpand" aria-expanded={expanded} onClick={() => toggleDomain(rowKey)}><span className="networkRouteExpandMark" aria-hidden="true">{expanded ? "−" : "+"}</span><span><strong>{domain.value}</strong><small>{domain.resolved.length ? `${domain.resolved.length} IP · раскрыть сведения` : "раскрыть сведения"}</small></span></button>{domain.role.includes("CDN") && <NetworkEch domain={domain.value} route={domain.route} request={request} />}</td><td><span>{domain.role}</span><small>{domain.source}</small></td><td><code>{domain.resolved.join(", ") || "Нет ответа"}</code></td><td><NetworkRouteTags domain={domain} /></td></tr>{expanded && <tr className="networkRouteDetailRow"><td colSpan={4}><div className="networkRouteCascade"><div className="networkRouteCascadeHeader"><span className="networkKicker">ДЕТАЛИ МАРШРУТА</span><strong>{domain.value}</strong></div><NetworkIdentityDetails domain={domain} /></div></td></tr>}</Fragment>; })}</tbody></table></div>{!domains.length && <p className="networkEmpty">{query ? "Совпадений нет." : "Маршруты ещё не настроены."}</p>}<details className="networkEvidence"><summary>Детали проверки</summary>{evidence.length ? <ul>{evidence.map((item) => <li key={item}>{item}</li>)}</ul> : <p>Дополнительных данных нет.</p>}</details></section>
