...
export function NetworkView({ request, refreshKey = 0 }: Props) {
  ...
  return <div data-network-page="true"><main className="networkBoard">
    <header className="networkHeader">
      <div className="networkHeading"><span className="networkLogo"><NetworkIcon /></span><div><span className="networkCaption">Инфраструктура / {status?.server.name || "VPS"}</span><h1>Сеть</h1><p>Доступ к серверу, маршруты и разрешение имён.</p></div></div>
...
function Diagnostics({ status }: { status: NetworkStatus }) {
  ...
  return <div className="networkDiagnostics">
    ...
    <section className="networkPanel"><div className="networkSectionHeading"><div><span className="networkCaption">04 / Службы</span><h2>Слушающие порты <span className="networkCount">{status.listeners.length}</span></h2><p>Локальные слушатели сервера. Наличие порта не означает доступность из интернета.</p></div></div>{status.listeners.length ? <div className="networkListeners">{status.listeners.map((listener, index) => <article key={`${listener.protocol}-${listener.port}-${index}`}><strong>{listener.port}<span>{listener.protocol}</span></strong><code>{listener.process || "Процесс не определён"}</code></article>)}</div> : <div className="networkEmpty compact"><p>Информация о слушающих портах отсутствует.</p></div>}</section>
  </div>;
}
