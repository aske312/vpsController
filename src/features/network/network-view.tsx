...
function NetworkWorkspace({ status, loading, onRefresh, dns, dnsDraft, dnsChecks, checkingDns, busy, setDnsDraft, checkDnsProviders, saveDnsSettings, mihomoDns, mihomoDnsDraft, mihomoDnsBusy, setMihomoDnsDraft, saveMihomoDns }: WorkspaceProps) {
  ...
  return <main className="networkBoard">
     ...
     <section className="networkControlLead" aria-labelledby="network-control-title"><div><Caption>CONTROL PLANE</Caption><h2 id="network-control-title">Настройка сетевого доступа</h2><p>Управляйте DNS для VPS, протоколов и Mihomo из одной точки. Изменения применяются после явного запуска.</p></div><div className="networkControlMeta"><span><Caption>ACTIVE PROFILE</Caption><strong>{dnsDraft?.profiles?.system || dnsDraft?.selected_id || "—"}</strong></span><span><Caption>CHANNELS</Caption><strong>{Object.values(dns?.protocol_effect_details || {}).filter((item) => item.installed).length + (mihomoDns ? 1 : 0)}</strong></span></div></section>
