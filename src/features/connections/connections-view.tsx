...
import type { Client, DeviceProbe, Protocol, ProtocolStatus } from "../../shared/types/control-plane";
...
type ConnectionsViewProps = {
  protocolStatuses: Partial<Record<Protocol, ProtocolStatus>>;
          ...
          <div className="connectionsRows">
            {protocolClients.length ? visibleClients.map((client) => {
              ...
              return <div className={`connectionRow quality-${client.quality || "offline"}`} key={client.id}>
                <div className="connectionIdentity"><span className={`protocol protocol-${client.protocol}`} title={protocolLabel}><ProtocolIcon protocol={client.protocol} /></span><div><strong>{client.name}</strong>{client.route_state?.state === "stale" && <small title={client.route_state.reason}>Stale · {client.route_state.addresses.join(", ")}</small>}{client.route_state?.state === "unknown" && <small title={client.route_state.reason}>Маршрут: Unknown</small>}<small>{client.address}{client.active_sources?.length ? `  ${client.active_sources.join(", ")}` : ""}</small></div></div>
                <div className="connectionState"><ProtocolHealthBadge health={protocolStatuses[client.protocol]?.health} /><small>{clientStateLabel}</small><small>{client.quality_reason || "состояние уточняется"}</small></div>
