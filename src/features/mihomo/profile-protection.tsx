...
export function ProfileProtection({ routing, connections, modules, common = false, client, onChange }: { routing: Routing; connections: ProfileConnection[]; modules: Module[]; common?: boolean; client?: string; onChange: (key: string, value: string | boolean) => void }) {
  ...
  const forceFeatures = ["tun_force", "dns_hijack_force", "tun_strict_route"] as const;
  const enabledCount = [...(common ? ["tunnel_privacy", "tunnel_ech", "tunnel_fragment"] : caps.features), ...(showMihomo ? [...mihomoFeatures.map(([key]) => key), ...forceFeatures] : [])].filter((key) => routing[key]).length;
  const totalCount = (common ? 3 : caps.features.length) + (showMihomo ? mihomoFeatures.length + forceFeatures.length : 0);
  ...
  return <section className="mihomoProfileRules">
    ...
    <div>
      ...
      {showMihomo && <>
        <button type="button" className={`mihomoProfileRuleButton${routing.tun_force ? " is-enabled" : ""}`} aria-pressed={Boolean(routing.tun_force)} onClick={() => onChange("tun_force", !routing.tun_force)}><i>TUN</i><span><b>TUN · принудительно</b><small>Включает TUN в конфигурации этого профиля или устройства.</small></span></button>
