...
class DnsReserveTests(unittest.TestCase):
    def setUp(self):
        self.providers = [
            ...
            {"id": "backup", "addresses": ["8.8.8.8", "8.8.4.4"]},
    ...
    def test_save_applies_backup_to_system_and_protocols(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            xray.assert_called_once_with(["https://primary.example/dns-query", "1.1.1.1", "8.8.8.8"])
    ...
    def test_status_compares_each_component_with_its_own_profile(self):
        ...
        with patch.object(api, "read_dns_settings", return_value=self.settings), patch.object(api, "dns_provider_list", return_value=self.providers), patch.object(api, "current_env_value", side_effect=lambda key, default: actual[key]), patch.object(api, "run", return_value="enabled"), patch.object(api, "system_dns_state", return_value={}):
