...
class TunnelPrivacyTests(unittest.TestCase):
    ...
    def test_saved_legacy_flag_is_repaired_while_previous_yaml_remains_valid(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(manager, "CONFIG_ROOT", config_root), patch.object(manager, "PROFILE_FILE", profile_file), patch.object(manager, "ROUTING_SETTINGS_FILE", root / "routing.json"), patch.object(manager, "profiles", side_effect=lambda: json.loads(profile_file.read_text())), patch.object(manager, "observe_service", return_value={"unit_present": True, "runtime": {"state": "stopped"}}), patch.object(manager, "systemctl_active", return_value=False), patch.object(manager, "write_action"), patch.object(manager, "vless_encryption_pair", return_value=("private", "public")), patch.object(manager, "render_profile", return_value="yaml"), patch.object(manager, "validate_rendered_profile"), patch.object(manager, "apply_batched_reality_runtime"), patch.object(manager, "apply_reality_config", side_effect=lambda path, value, **_: path.write_text(json.dumps(value))), patch.object(manager, "provision") as provision:
    ...
    def test_device_only_update_rolls_back_profile_and_runtime_on_failure(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(manager, "CONFIG_ROOT", config_root), patch.object(manager, "PROFILE_FILE", profile_file), patch.object(manager, "ROUTING_SETTINGS_FILE", root / "routing.json"), patch.object(manager, "profiles", side_effect=lambda: json.loads(profile_file.read_text())), patch.object(manager, "observe_service", return_value={"unit_present": True, "runtime": {"state": "stopped"}}), patch.object(manager, "systemctl_active", return_value=False), patch.object(manager, "validate_routing", side_effect=lambda values, **_: values), patch.object(manager, "write_action"), patch.object(manager, "vless_encryption_pair", return_value=("private", "public")), patch.object(manager, "render_profile", return_value="yaml"), patch.object(manager, "validate_rendered_profile"), patch.object(manager, "apply_reality_config", side_effect=fail), patch.object(manager, "provision") as provision:
    ...
    def test_failed_global_toggle_restores_setting_profiles_and_server_config(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(manager, "CONFIG_ROOT", config), patch.object(manager, "PROFILE_FILE", profile), patch.object(manager, "ROUTING_SETTINGS_FILE", routing), patch.object(manager, "routing_settings", return_value={"tunnel_privacy": False}), patch.object(manager, "validate_routing", return_value={"tunnel_privacy": True}), patch.object(manager, "observe_service", return_value={"unit_present": True, "runtime": {"state": "stopped"}}), patch.object(manager, "systemctl_active", return_value=False), patch.object(manager, "apply_tunnel_privacy", side_effect=fail):
