...
test("Mihomo transports automatically provision DNS and routing policies", async () => {
  ...
  assert.match(styles, /\.mihomoCommandHero \.mihomoHeroArt \{[^}]*position:absolute[^}]*background-color:#01010e[^}]*background-image:url\("\/gate-art\/new-operator\/mihomo-control-v2\.png"\)[^}]*background-size:contain[^}]*drop-shadow/s);
...
test("direct OpenVPN is installable and uses per-client PKI with CRL revocation", async () => {
  const [manifestText, install, firewall, uninstall, api, view] = await Promise.all([
    ...
    read("api/main.py"), read("app/views/protocols/protocol-view.tsx"),
  ]);
  const manifest = JSON.parse(manifestText);
