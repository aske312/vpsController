...
export default function Home() {
  ...
  const connectionTypeOptions = installedProtocols.flatMap((protocol) => {
    if (protocol !== "vless-reality-xhttp") return [{
      ...
      badge: protocol === "shadowsocks" ? "SS" : protocol === "hysteria2" ? "HY2" : protocol.toUpperCase(),
  ...
  >
      ...
      {(tab === "channels" || tab === "dns") && <nav className="protocolSwitcher channelPageSwitcher" aria-label="Защищённые каналы">
        {installedProtocols.map((protocol) => <button type="button" key={protocol} className={tab === "channels" && selectedChannel === protocol ? "active" : ""} onClick={() => { setSelectedChannel(protocol); setTab("channels"); void loadProtocolStatus(protocol); }}>{protocol === "wg" ? "WG" : protocol === "awg" ? "AWG" : protocol === "shadowsocks" ? "SS" : protocol === "hysteria2" ? "HY2" : "VLESS"}</button>)}
