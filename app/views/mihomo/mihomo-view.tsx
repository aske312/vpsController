      ...
      {profileDialog && (
        <div className="mihomoDialogBackdrop mihomoProfileBackdrop" onMouseDown={(event) => {
          ...
          <form className={`mihomoDialog mihomoProfileDialog is-step-${profileStep}`} onSubmit={saveProfile}>
            ...
            <section className="mihomoProfileRules"><header><div><b>Защита соединений</b><small>Применяется ко всем каналам выбранного профиля.</small></div></header><div className="mihomoDnsAdvanced"><label className={activeProfileRouting.tunnel_privacy ? "is-enabled" : ""}><span><b>Усиленная защита</b><small>Шифрование VLESS, DoH и ECH при поддержке CDN</small></span><input type="checkbox" checked={Boolean(activeProfileRouting.tunnel_privacy)} onChange={(event) => { const enabled = event.target.checked; setProfileRouting((current) => ({ ...current, tunnel_privacy: enabled })); setProfileDevices((current) => current.map((device) => device.id === activeDeviceId ? { ...device, routing: { ...(device.routing || {}), tunnel_privacy: enabled } } : device)); }} /></label><p className="mihomoGamesHint">Для VLESS нужен Mihomo 1.19.30+. Другие протоколы сохраняют штатное шифрование, DIRECT-маршруты не меняются. После сохранения обновите подписку.</p></div></section>
