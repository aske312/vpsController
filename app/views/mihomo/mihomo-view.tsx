...
type Profile = {
  ...
  common_access?: { os?: string; client_name?: string; client_version?: string; user_agent?: string; last_seen_at?: string; reason?: "hwid_missing" };
  created_at: string;
  updated_at: string;
};
            ...
            {profiles.map((profile) => { const allDevices = profile.devices?.length ? profile.devices : [{ id: "profile-common", name: "Общие настройки профиля", scope: "common" as const }]; const devices = registeredProfileDevices(profile); const commonDevice = allDevices.find((device) => device.scope === "common" || device.id === profile.common_device_id) || allDevices[0]; const stats = profileStats[profile.id]?.summary; const ruleCount = profileDirectRules.filter((rule) => Boolean(profile.routing?.[rule.key])).length; return (
              <section className="mihomoProfileCard" key={profile.id}>
                ...
                {expandedDeviceLists.has(profile.id) && <div className="mihomoProfileDevices">
                  {profile.common_access?.reason === "hwid_missing" && <div className="mihomoCommonAccessNotice"><b>Последний клиент общего пула</b><span>{devicePlatform[profile.common_access.os || "unknown"]?.label || "Неизвестная ОС"} · {profile.common_access.client_name || "неизвестный клиент"}{profile.common_access.client_version ? ` ${profile.common_access.client_version}` : ""}</span><small>HWID не передан — персональное устройство и отдельные credentials не создавались{profile.common_access.last_seen_at ? ` · ${new Date(profile.common_access.last_seen_at).toLocaleString("ru-RU")}` : ""}</small></div>}
                  {devices.map((device) => {
