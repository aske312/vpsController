...
```bash
curl -fsSL https://raw.githubusercontent.com/aske312/vpsController/stabl/scripts/install-panel.sh | sudo bash -s -- --domain vpn.example.com
```
...
GeoIP определяет регистрацию IP-подсети, а не местонахождение оборудования. Поэтому одиночная база может показать город владельца адресного блока вместо дата-центра. Универсального сетевого способа достоверно определить физическое размещение на чистом VPS нет.

По умолчанию установщик сначала использует регион из metadata/cloud-init, если провайдер его публикует. Если metadata отсутствуют, он сравнивает несколько сетевых источников. Страна принимается при согласии как минимум двух источников, а город — только при отдельном подтверждении. Спорное значение не выдаётся за физическую локацию.

Необязательный ручной override нужен только когда провайдер не публикует metadata, а автоматические источники расходятся. Значения следует брать из заказа или панели хостинга:
...
sudo bash ./install-312.sh \
  ...
  --location-country-code XX
```

Если название содержит пробелы, заключите его в кавычки. Override сохраняется в `/etc/vps-control.env`, имеет приоритет над автоматическим определением и не перезаписывается обновлениями.
