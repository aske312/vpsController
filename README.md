...
Без параметров установщик сам создаёт случайный пароль администратора и показывает адрес, логин и пароль только в итоговом сообщении. Необязательные параметры старта:

- `VPS_CONTROL_ADMIN_USER` — логин панели;
- `VPS_CONTROL_ADMIN_PASSWORD` — заранее выбранный пароль; если не задан, создаётся автоматически;
- `VPS_CONTROL_PUBLIC_DOMAIN` — домен панели; если пусто, проверяется PTR и его A/AAAA;
- `VPS_CONTROL_ACCESS_MODE` — `external`, `local` или `vpn`;
- `VPS_CONTROL_HTTP_PORT` — HTTP-порт при работе без домена, по умолчанию `8080`;
- `VPS_CONTROL_VLESS_PORT` — порт VLESS Reality, по умолчанию `8443`; порт 443 зарезервирован для HTTPS;
- `VPS_CONTROL_BRANCH` — ветка загрузки, для пользователей следует оставлять `stabl`;
- `VPS_CONTROL_PREFLIGHT_ONLY=yes` — выполнить диагностику без развёртывания приложения.

Пример установки с заранее выбранным доменом и учётной записью:

```bash
curl -fsSL https://raw.githubusercontent.com/aske312/vpsController/stabl/scripts/install-panel.sh | \
  sudo VPS_CONTROL_PUBLIC_DOMAIN=panel.example.com \
  VPS_CONTROL_ADMIN_USER=operator \
  VPS_CONTROL_ADMIN_PASSWORD='замените-на-длинный-пароль' bash
```

Все остальные решения принимаются автоматически: определяется ОС и архитектура, выбирается подходящий источник Node.js 22, устанавливаются зависимости, проверяются IPv4/IPv6 и DNS, настраиваются UFW и Caddy, выпускается TLS-сертификат и проверяются службы. Повторный запуск безопасно завершает прерванную установку и сохраняет существующие настройки.
