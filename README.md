...
Обновление приложения также обеспечивает версии необходимых системных зависимостей.
Проверенный пакет Caddy входит в релиз; требования и контрольные суммы находятся в
`api/runtime-dependencies.json`. Перед установкой проверяется совместимость текущей
конфигурации, локальные настройки сохраняются, более новая версия Caddy не понижается.
Зависимости могут остаться обновлёнными при откате кода приложения. Полное обновление
ОС выполняется отдельно. Кнопка обновления ядра использует установленные метапакеты
Debian/Ubuntu, включая HWE и облачные варианты, без смены семейства ядра.
